...
# Ignore ssl warnings, if verify is set to false
...
def post(url, data):
   if data:
      r = requests.post(url, data=json.dumps(data), auth=(USERNAME, PASSWORD), verify=VERIFY_SSL)
   else:
      r = requests.post(url, data="", auth=(USERNAME, PASSWORD), verify=VERIFY_SSL)
   r.raise_for_status()
...
@baker.command(params={"service_id": "The ID of the service to finish upgrade."})
def finishupgrade(service_id):
    r = get(HOST + URL_SERVICE + service_id)
    current_service_config = r.json()
...
@baker.command(params={"service_id": "The ID of the service to rollback."})
def rollback(service_id):
    r = get(HOST + URL_SERVICE + service_id)
    current_service_config = r.json()

    post(HOST + URL_SERVICE + service_id + "?action=rollback", "")
    r = get(HOST + URL_SERVICE + service_id)
...
if __name__ == '__main__':
   ...
   if 'RANCHER_VERIFY_SSL' in os.environ:
      VERIFY_SSL = str2bool(os.environ['RANCHER_VERIFY_SSL'])
